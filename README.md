...
- **N1MM / logger WinKeyer input could hang permanently.** After some use, the logger's WinKeyer indicator would turn red and stay red even after restarting the logger — only a Station restart recovered it. The root cause (an over-eager virtual-port disconnect detector that reset the session mid-transmission) has been removed. Reconnects are now handled non-destructively (a stale half-command is cleared after an inter-byte gap, and Host Open re-establishes the session cleanly), and the port only resets on a genuine I/O error.
- **Logger macros did not key the radio in some cases.** The keyer engine could carry a stuck state between sessions, so text was received but never keyed. The engine is now reset at the start of every logger session, and a transient keying error can no longer silently kill the keyer thread for the rest of the session.
- **CW glitched / dahs dragged out.** The keyer timing thread was busy-spinning a CPU core at highest priority for the full length of every element, starving the audio and networking threads. It now sleeps between edges and only spins for the final sub-millisecond, so CW is smooth while keeping timing accuracy.
- **Log files were effectively lost after the Program Files move.** Logs were written next to the executable (not writable by a normal user), so they silently failed or were redirected by Windows. All logs now live in `%LocalAppData%\RWK Router Keyer\logs`, and both apps have a **View Logs** menu that opens each log in Notepad.
- **Station Key and PTT could be set to the same line.** Selecting a PTT line that matches the Key line (both DTR or both RTS) is invalid and prevented the keying port from opening. The Station now automatically sets PTT to (None) in that case.
